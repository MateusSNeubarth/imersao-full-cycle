...
import (
	"github.com/MateusSNeubarth/imersao-full-cycle/internal/events/domain"
...
type BuyTicketsOutputDTO struct {
	Tickets []TicketsDTO `json:"tickets"`
}
...
func (uc *BuyTicketsUseCase) Execute(input BuyTicketsInputDTO) (*BuyTicketsOutputDTO, error) {
	...
	ticketDTOs := make([]TicketsDTO, len(tickets))
	for i, ticket := range tickets {
		ticketDTOs[i] = TicketsDTO{
			ID:         ticket.ID,
